...
const { sideLength, boxNum, opacity, size, rangeX, rangeY, rangeZ, pos, isAnimating, actBox, zoom } = toRefs(data)
...
const initPos = () => {
  ...
  // }
  for (let z = 0; z < sideLength.value; z++) {
    for (let y = 0; y < sideLength.value; y++) {
      for (let x = 0; x < sideLength.value; x++) {
        pos.value.push({ x, y, z })
      }
    }
  }
...
.bigFace .arrow {
  ...
  transform: translate3d(260px, 260px, 150px) rotateX(90deg);
}
.bigFace.floor .arrow {
  transform: translate3d(260px, 260px, -150px) rotateX(90deg) rotateX(180deg);
}
.bigFace:hover {
  background: red;
}
